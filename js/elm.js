...
Elm.Clock.make = function (_elm) {
   ...
   if (_elm.Clock.values)
   return _elm.Clock.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $StartStopButtons = Elm.StartStopButtons.make(_elm),
   $Time = Elm.Time.make(_elm),
   $TimerLengthButtons = Elm.TimerLengthButtons.make(_elm),
   $Types = Elm.Types.make(_elm);
   var formatTime = function (t) {
      return function () {
         var minutes = $Basics.floor($Time.inMinutes(t));
         var minutesString = A2($Basics._op["++"],
         _U.cmp(minutes,
         10) < 0 ? "0" : "",
         ...
         60);
         var secondsString = A2($Basics._op["++"],
         _U.cmp(seconds,
         10) < 0 ? "0" : "",
   ...
   var view = function (model) {
      ...
      _L.fromArray([$Html$Attributes.$class("center-block text-center bg-danger")
                   ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                         ,_0: "font-size"
                                                         ,_1: "48px"}
                                                        ,{ctor: "_Tuple2"
                                                        ...
                                                        ,{ctor: "_Tuple2"
                                                         ,_0: "color"
                                                         ,_1: "lightgray"}]))]),
      _L.fromArray([$Html.text(formatTime(model.time))]))]))]));
   };
   var Tick = function (a) {
      return {ctor: "Tick",_0: a};
   };
   var signal = $Time.timestamp(A2($Signal.map,
   function ($) {
      return Tick($Basics.always(1 * $Time.second)($));
   },
   ...
   var init = function (initialTime) {
      return {_: {}
             ,state: Stopped
             ,time: initialTime};
   };
   var updateTimerLength = F2(function (timerLengthButtonsAction,
   model) {
      return function () {
         switch (timerLengthButtonsAction.ctor)
         {case "ClickLongBreak":
            return init($Types.longLength);
            case "ClickPomodoro":
            return init($Types.pomodoroLength);
            case "ClickShortBreak":
            return init($Types.shortLength);}
         _U.badCase($moduleName,
   ...
   model) {
      return function () {
         ...
         {case "Tick":
            return function () {
                 var updatedTime = model.time - action._0;
                 var hasEnded = _U.cmp(updatedTime,
                 1) < 1;
         ...
         _U.badCase($moduleName,
         "between lines 34 and 48");
      }();
   });
   var updateClockState = F2(function (startStopButtonsAction,
   model) {
      return function () {
         var oldState = model.state;
         return function () {
            switch (startStopButtonsAction.ctor)
            {case "ClickStart":
               return _U.replace([["state"
                                  ,_U.eq(oldState,
                                  Stopped) ? Running : oldState]],
                 model);
               case "ClickStop":
               return _U.replace([["state"
                                  ,_U.eq(oldState,
                                  Running) ? Stopped : oldState]],
                 model);}
            _U.badCase($moduleName,
...
Elm.Main.make = function (_elm) {
   ...
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $PlaySound = Elm.PlaySound.make(_elm),
   $PomodoroList = Elm.PomodoroList.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $StartStopButtons = Elm.StartStopButtons.make(_elm),
   $Time = Elm.Time.make(_elm),
   $TimerLengthButtons = Elm.TimerLengthButtons.make(_elm),
      ...
      return function () {
         ...
         {case "ClockAction":
            switch (action._0.ctor)
              {case "_Tuple2":
                 return function () {
                      var timerLengthType = $TimerLengthButtons.getTimerLengthType(model.timerLengthButtons);
                      var $ = A2($Clock.update,
                      action._0._1,
                      model.clock),
                      newClock = $._0,
                      newHasEnded = $._1;
                      var newPlaySound = A2($PlaySound.update,
                      newHasEnded,
                      model.player);
                      var oldHasEnded = model.countdownHasEnded;
                      var newPomodoroList = _U.eq(newHasEnded,
                      true) && _U.eq(oldHasEnded,
                      false) ? A4($PomodoroList.update,
                      newHasEnded,
                      timerLengthType,
                      action._0._0,
                      model.pomodoroList) : model.pomodoroList;
            ...
            case "StartStopButtonsAction":
            return _U.replace([["clock"
                               ,A2($Clock.updateClockState,
                               action._0,
                               model.clock)]],
              model);
            case "TimerLengthButtonsAction":
            return function () {
                 var newTimerLengthButtons = A2($TimerLengthButtons.update,
                 action._0,
   ...
   var StartStopButtonsAction = function (a) {
      return {ctor: "StartStopButtonsAction"
             ,_0: a};
   };
   var TimerLengthButtonsAction = function (a) {
      return {ctor: "TimerLengthButtonsAction"
             ,_0: a};
   };
   var ClockAction = function (a) {
      return {ctor: "ClockAction"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   ...
   var view = function (model) {
      return function () {
         var startStopButtonsContext = $StartStopButtons.Context(A2($Signal.forwardTo,
         actionMailbox.address,
         StartStopButtonsAction));
         var timerLengthButtonsContext = $TimerLengthButtons.Context(A2($Signal.forwardTo,
         actionMailbox.address,
   ...
   var allSignals = $Signal.mergeMany(_L.fromArray([A2($Signal.map,
                                                   ClockAction,
                                                   $Clock.signal)
                                                   ,actionMailbox.signal]));
   var emptyModel = {_: {}
                    ,clock: $Clock.init($Types.pomodoroLength)
                    ,countdownHasEnded: false
   ...
   false)($Signal.dropRepeats($Signal.map(function (_) {
      return _.countdownHasEnded;
   })(model)))));
   ...
   _elm.Main.values = {_op: _op
                      ...
                      ,TimerLengthButtonsAction: TimerLengthButtonsAction
                      ,StartStopButtonsAction: StartStopButtonsAction
                      ,update: update
                      ,view: view
                      ,main: main
                      ,model: model
...
Elm.PlaySound = Elm.PlaySound || {};
Elm.PlaySound.make = function (_elm) {
   "use strict";
   _elm.PlaySound = _elm.PlaySound || {};
   if (_elm.PlaySound.values)
   return _elm.PlaySound.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
...
Elm.PomodoroList = Elm.PomodoroList || {};
Elm.PomodoroList.make = function (_elm) {
   "use strict";
   _elm.PomodoroList = _elm.PomodoroList || {};
   if (_elm.PomodoroList.values)
   return _elm.PomodoroList.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "PomodoroList",
   $Basics = Elm.Basics.make(_elm),
   ...
   $Time = Elm.Time.make(_elm),
   $Types = Elm.Types.make(_elm);
   var dayEnding = function (dayNumber) {
      return function () {
   ...
   var viewDate = function (date) {
      return function () {
         var min = $Basics.toString($Date.minute(date));
         var hour = $Basics.toString($Date.hour(date));
         var dayEnding$ = dayEnding($Date.day(date));
         var day = $Basics.toString($Date.day(date));
         var month = viewMonth($Date.month(date));
         return A2($Basics._op["++"],
         month,
         A2($Basics._op["++"],
         " ",
   ...
   var viewOneLine = function (item) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("row")]),
      _L.fromArray([A2($Html.span,
                   _L.fromArray([$Html$Attributes.$class("col-md-6")]),
                   _L.fromArray([$Html.text($Types.pomodoroTypeToString(item.pomodoroType))]))
                   ,A2($Html.span,
                   _L.fromArray([$Html$Attributes.$class("col-md-6")]),
                   _L.fromArray([$Html.text(viewDate(item.pomodoroDate))]))]));
   };
   var view = function (model) {
      return function () {
         var title = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("row")]),
         _L.fromArray([A2($Html.h4,
                      ...
                      ,A2($Html.h4,
                      _L.fromArray([$Html$Attributes.$class("col-md-6")]),
                      _L.fromArray([$Html.text("Date")]))]));
         var listItems = A2($List.map,
         viewOneLine,
         model.pomodoroList);
         var entireList = _U.cmp($List.length(listItems),
         0) > 0 ? A2($List._op["::"],
         title,
         listItems) : _L.fromArray([]);
         return A2($Html.div,
         _L.fromArray([]),
         entireList);
      }();
   ...
   var init = {_: {}
              ,pomodoroList: _L.fromArray([])};
   var Model = function (a) {
      return {_: {}
             ,pomodoroList: a};
   };
   var PomodoroListItem = F2(function (a,
   b) {
      return {_: {}
             ,pomodoroDate: b
             ,pomodoroType: a};
   ...
   var maxItems = 20;
   var updatePomodoroList = F3(function (pomodoroType,
   currentTime,
   model) {
      return function () {
         var newPomodoro = {_: {}
         ...
         var newPomodoroList = $List.take(maxItems)(A2($List._op["::"],
         newPomodoro,
         model.pomodoroList));
         return _U.replace([["pomodoroList"
                            ,newPomodoroList]],
         model);
      }();
   });
   var update = F4(function (hasEnded,
   pomodoroType,
   currentTime,
   model) {
      return hasEnded ? A3(updatePomodoroList,
      pomodoroType,
      currentTime,
      model) : model;
   });
   _elm.PomodoroList.values = {_op: _op
                              ,init: init
                              ,update: update
                              ,view: view
                              ,Model: Model};
...
Elm.StartStopButtons = Elm.StartStopButtons || {};
Elm.StartStopButtons.make = function (_elm) {
   "use strict";
   _elm.StartStopButtons = _elm.StartStopButtons || {};
   if (_elm.StartStopButtons.values)
   return _elm.StartStopButtons.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "StartStopButtons",
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   ...
   var ClickStop = {ctor: "ClickStop"};
   var ClickStart = {ctor: "ClickStart"};
   var view = function (context) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("row")]),
      _L.fromArray([A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("col-md-3 col-md-offset-3")]),
                   _L.fromArray([A2($Html.button,
                   _L.fromArray([A2($Html$Events.onClick,
                                context.clickChannel,
                                ClickStart)
                                ,$Html$Attributes.$class("btn btn-success btn-block btn-start-stop")]),
                   ...
                   _L.fromArray([A2($Html$Events.onClick,
                                context.clickChannel,
                                ClickStop)
                                ,$Html$Attributes.$class("btn btn-danger btn-block btn-start-stop")]),
                   _L.fromArray([$Html.text("Stop")]))]))]));
   };
   _elm.StartStopButtons.values = {_op: _op
                                  ,view: view
                                  ,Context: Context
...
Elm.TimerLengthButtons = Elm.TimerLengthButtons || {};
Elm.TimerLengthButtons.make = function (_elm) {
   "use strict";
   _elm.TimerLengthButtons = _elm.TimerLengthButtons || {};
   if (_elm.TimerLengthButtons.values)
   return _elm.TimerLengthButtons.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "TimerLengthButtons",
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $Html$Events = Elm.Html.Events.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Types = Elm.Types.make(_elm);
   var createButton = F3(function (context,
   action,
   buttonText) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("col-md-4")]),
      ...
      _L.fromArray([A2($Html$Events.onClick,
                   context.clickChannel,
                   action)
                   ,$Html$Attributes.$class("btn btn-primary btn-block btn-pomodoro-type")]),
   ...
   model) {
      return function () {
         switch (action.ctor)
         {case "ClickLongBreak":
            return _U.replace([["pomodoroType"
                               ,$Types.LongBreak]],
              model);
            case "ClickPomodoro":
            return _U.replace([["pomodoroType"
   ...
   var getTimerLengthType = function (model) {
      return model.pomodoroType;
   };
   var init = {_: {}
              ,pomodoroType: $Types.Pomodoro};
   var ClickLongBreak = {ctor: "ClickLongBreak"};
   var ClickShortBreak = {ctor: "ClickShortBreak"};
   var ClickPomodoro = {ctor: "ClickPomodoro"};
   var view = function (context) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("row")]),
      _L.fromArray([A3(createButton,
                   context,
                   ClickPomodoro,
                   "Pomodoro")
                   ,A3(createButton,
   ...
   _elm.TimerLengthButtons.values = {_op: _op
                                    ,getTimerLengthType: getTimerLengthType
                                    ,init: init
                                    ,update: update
                                    ,view: view
                                    ,Model: Model
                                    ,Context: Context
                                    ,ClickPomodoro: ClickPomodoro
                                    ,ClickShortBreak: ClickShortBreak
                                    ,ClickLongBreak: ClickLongBreak};
   return _elm.TimerLengthButtons.values;
};
...
Elm.Types.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Time = Elm.Time.make(_elm);
   var debugShortTimes = false;
   var pomodoroLength = debugShortTimes ? 25 * $Time.second : 25 * $Time.minute;
   var shortLength = debugShortTimes ? 5 * $Time.second : 5 * $Time.minute;
   var longLength = debugShortTimes ? 10 * $Time.second : 10 * $Time.minute;
   var pomodoroTypeToString = function (pomodoroType) {
      return function () {
         switch (pomodoroType.ctor)
         {case "LongBreak":
            return "Long Break";
            case "Pomodoro":
            return "Pomodoro";
   ...
   _elm.Types.values = {_op: _op
                       ...
                       ,pomodoroTypeToString: pomodoroTypeToString
